...
class Player(Ship):#defining the player class inheriting from ship
    ####################################################1
    #these constants define the minimum and maximum sizes and speeds for the player ship.
    #initial values are set to ensure the ship starts at the minimum size and speed. 
    MIN_SIZE = 50
    MAX_SIZE = 100
    MIN_SPEED = 5
    MAX_SPEED = 10
    ...
    def __init__(self, x, y, health=100):#initialization method
        ...
        self.ship_img = pygame.transform.scale(YELLOW_SPACE_SHIP, (self.MIN_SIZE, self.MIN_SIZE))#setting the ship image######2
        ...
        self.move_speed = self.MIN_SPEED#setting the initial movement speed#########3
    ...
    def draw(self, window):#method to draw the player
        ...
        size_label = main_font.render(f"Size: {self.get_width()}", 1, font_color)#######4
        speed_label = main_font.render(f"Speed: {self.move_speed}", 1, font_color)#######5
        window.blit(size_label, (10, 100))
        window.blit(speed_label, (10, 130))
    ...
    def take_damage(self, amount):#method to handle damage
        ...
        if self.health > 0:#only decrease firing speed if the player is still alive
            ...
            ###################################################6
            #when the player takes damage, the ship size is increased, but not beyond the maximum size,
            #and the movement speed is decreased, but not below the minimum speed.
            new_size = min(self.get_width() + 5, self.MAX_SIZE)
            self.ship_img = pygame.transform.scale(YELLOW_SPACE_SHIP, (new_size, new_size))
            self.mask = pygame.mask.from_surface(self.ship_img)
            self.move_speed = max(self.MIN_SPEED, self.move_speed - 1)#reducing the movement speed but ensuring it doesn't go below the minimum
            ###################################################
            if sound_enabled:
                sfx_healDown.play()#playing the damage sound

    def apply_reward(self, reward_type):#method to apply rewards
        if reward_type == 'health':
            self.health = min(self.max_health, self.health + 20)
            if sound_enabled:
                sfx_healUp.play()
        elif reward_type == 'cooldown':
            self.firing_speed = min(6, self.firing_speed + 1)
            self.COOLDOWN = 30 // self.firing_speed
            #################################################7
            #When a cooldown reward is applied, the ship size is decreased, but not below the minimum size,
            #and the movement speed is increased, but not beyond the maximum speed.
            new_size = max(self.get_width() - 5, self.MIN_SIZE)
            self.ship_img = pygame.transform.scale(YELLOW_SPACE_SHIP, (new_size, new_size))
            self.mask = pygame.mask.from_surface(self.ship_img)
            self.move_speed = min(self.MAX_SPEED, self.move_speed + 1)#increasing the movement speed but ensuring it doesn't go above the maximum
            #################################################
            if sound_enabled:
                sfx_shootingSpeedUp.play()
...
class RewardEnemy(Enemy):#defining the reward enemy class inheriting from enemy
    ...
    def apply_reward(self, player):#method to apply the reward to the player
        player.apply_reward(self.reward_type)
...
def main():#main function to run the game
    ...
    while run:#game loop
        ...
        #########################################################8
        #these lines ensure that the player's movement speed is updated based on the current speed value of the player ship,
        #reflecting changes from rewards or damage.
        if keys[pygame.K_a] and player.x - player.move_speed > 0:#moving the player left
            player.x -= player.move_speed#updating the x coordinate
        if keys[pygame.K_d] and player.x + player.move_speed + player.get_width() < WIDTH:#moving the player right
            player.x += player.move_speed#updating the x coordinate
        if keys[pygame.K_w] and player.y - player.move_speed > 0:#moving the player up
            player.y -= player.move_speed#updating the y coordinate
        if keys[pygame.K_s] and player.y + player.move_speed + player.get_height() + 15 < HEIGHT:#moving the player down
            player.y += player.move_speed#updating the y coordinate
        ##########################################################
...
#Added constants for size and speed limits.
#Set initial size and speed to minimum values in the player class.
#Display size and speed information on the screen.
#Adjusted size and speed based on taking damage or applying rewards.
#Updated player movement handling to use dynamic speed values.